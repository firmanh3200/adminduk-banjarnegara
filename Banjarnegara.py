...
st.set_page_config(page_title='Adminduk Banjarnegara', layout='wide')

with st.container(border=True):
    with st.container(border=True):
        st.title(':blue[Administrasi Kependudukan] :green[Kabupaten Banjarnegara] :orange[Semester 1 2024]')
        st.caption('Sumber: https://gis.dukcapil.kemendagri.go.id/peta/')

st.subheader('', divider='orange')
kola, kolb, kolc, kold = st.columns(4)
with kola:
    with st.container(border=True):
        st.title('1.144,89')
        st.caption('Km2')
...
with st.container(border=True):
    st.subheader(f'Jumlah Penduduk: 1.071.977 Jiwa')
...
with st.container(border=True):
    st.subheader(f'Jumlah Kepala Keluarga: 359.857')
...
with tab1:
    ...
    lingkar1 = px.sunburst(jeniskelamin3, path=['Kabupaten/Kota', 'Kecamatan', 'Jenis Kelamin'], values='Jumlah Penduduk')
    ...
    with st.container(border=True):
        ...
        with kol2:
            st.plotly_chart(lingkar1, use_container_width=True)
...
with tab2:
    ...
    lingkar2 = px.sunburst(statuskawin, path=['Kabupaten/Kota', 'Kecamatan', 'Status Kawin'], values='Jumlah Penduduk')
    with st.container(border=True):
        kol1, kol2, = st.columns(2)
        with kol1:
            st.plotly_chart(trimep2, use_container_width=True)
        with kol2:
            st.plotly_chart(lingkar2, use_container_width=True)
        with st.expander('Unduh Tabel'):
...
with tab3:
    ...
    lingkar3 = px.sunburst(agama, path=['Kabupaten/Kota', 'Kecamatan', 'Agama'], values='Jumlah Penduduk')
    with st.container(border=True):
        kol1, kol2, = st.columns(2)
        with kol1:
            st.plotly_chart(trimep3, use_container_width=True)
        with kol2:
            st.plotly_chart(lingkar3, use_container_width=True)
        with st.expander('Unduh Tabel'):
...
with tab4:
    ...
    lingkar4 = px.sunburst(umur, path=['Kabupaten/Kota', 'Kecamatan', 'Kelompok Umur'], values='Jumlah Penduduk')
    with st.container(border=True):
        kol1, kol2, = st.columns(2)
        with kol1:
            st.plotly_chart(trimep4, use_container_width=True)
        with kol2:
            st.plotly_chart(lingkar4, use_container_width=True)
        with st.expander('Unduh Tabel'):
...
with tab5:
    ...
    lingkar5 = px.sunburst(pendidikan, path=['Kabupaten/Kota', 'Kecamatan', 'Pendidikan'], values='Jumlah Penduduk')
    with st.container(border=True):
        kol1, kol2, = st.columns(2)
        with kol1:
            st.plotly_chart(trimep5, use_container_width=True)
        with kol2:
            st.plotly_chart(lingkar5, use_container_width=True)
        with st.expander('Unduh Tabel'):
...
with tab6:
    ...
    lingkar6 = px.sunburst(goldar, path=['Kabupaten/Kota', 'Kecamatan', 'Golongan Darah'], values='Jumlah Penduduk')
    with st.container(border=True):
        kol1, kol2, = st.columns(2)
        with kol1:
            st.plotly_chart(trimep6, use_container_width=True)
        with kol2:
            st.plotly_chart(lingkar6, use_container_width=True)
        with st.expander('Unduh Tabel'):
...
with tab7:
    ...
    lingkar7 = px.sunburst(usia, path=['Kabupaten/Kota', 'Kecamatan', 'Usia Pendidikan'], values='Jumlah Penduduk')
    with st.container(border=True):
        kol1, kol2, = st.columns(2)
        with kol1:
            st.plotly_chart(trimep7, use_container_width=True)
        with kol2:
            st.plotly_chart(lingkar7, use_container_width=True)
        with st.expander('Unduh Tabel'):
...
with tab8:
    ...
    lingkar8 = px.sunburst(pekerjaan, path=['Kabupaten/Kota', 'Kecamatan', 'Pekerjaan'], values='Jumlah Penduduk')
    with st.container(border=True):
        kol1, kol2, = st.columns(2)
        with kol1:
            st.plotly_chart(trimep8, use_container_width=True)
        with kol2:
            st.plotly_chart(lingkar8, use_container_width=True)
        with st.expander('Unduh Tabel'):
